...
void errorCallback(geometry_msgs::TwistStamped error_msg) {
  ...
  ResidualError << endl
                << error_msg.header.stamp - prev_error_msg.header.stamp << "\t"
                << error_msg.twist.linear.x;
  ResidualError.close();
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber odom_sub = n.subscribe<geometry_msgs::TwistStamped>(
      "/ResidualError", 50, errorCallback);
