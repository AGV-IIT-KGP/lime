...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "translate_points");
  ros::NodeHandle node;
  ros::Rate rate(1000.0);
  tf::TransformListener listener;

  // Subscriber callback
  // Retrieves LiDAR point cloud from the /velodyne_points topic (frame_id =
  // velodyne_points)
  ros::Subscriber sub = node.subscribe("/velodyne_points", 1000, get_cloud);

  ros::Publisher cloud_pub =
      node.advertise<sensor_msgs::PointCloud2>("/shifted_points", 1000);
  ...
  while (node.ok()) {
    ...
    // Make a 3x3 matrix containing the rotation information of the Corrected
    // transform
    ...
    // Considering a 3dof localisation, roll and pitch matching are
    // insignificant and sometimes create divergence
    r = rolloff;
    p = pitchoff;
    ...
    pcl_ros::transformPointCloud("odom", get_tf_from_stamped_tf(transform),
                                 pcin, pcout);
