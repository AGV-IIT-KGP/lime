...
// ugly test depending on roscpp because tf_conversions is not properly
// versionized
...
transformListenerToEigenMatrix(const tf::TransformListener &listener,
...
transformListenerToEigenMatrix<float>(const tf::TransformListener &listener,
                                      const std::string &target,
                                      const std::string &source,
                                      const ros::Time &stamp);
template PointMatcher<double>::TransformationParameters
transformListenerToEigenMatrix<double>(const tf::TransformListener &listener,
                                       const std::string &target,
                                       const std::string &source,
                                       const ros::Time &stamp);
...
// Eigen to Odom
template <typename T>
nav_msgs::Odometry eigenMatrixToOdomMsg(
  ...
  // Fill pose
  const Eigen::Affine3d eigenTr(Eigen::Matrix4d(
      eigenMatrixToDim<double>(inTr.template cast<double>(), 4)));
...
template nav_msgs::Odometry eigenMatrixToOdomMsg<float>(
    const PointMatcher<float>::TransformationParameters &inTr,
    const std::string &frame_id, const ros::Time &stamp);
template nav_msgs::Odometry eigenMatrixToOdomMsg<double>(
...
template <typename T>
typename PointMatcher<T>::TransformationParameters
poseMsgToEigenMatrix(const geometry_msgs::Pose &pose) {
  Eigen::Affine3d eigenTr;
...
template PointMatcher<float>::TransformationParameters
poseMsgToEigenMatrix<float>(const geometry_msgs::Pose &pose);
template PointMatcher<double>::TransformationParameters
poseMsgToEigenMatrix<double>(const geometry_msgs::Pose &pose);

//--------------------------------
// Eigen to Pose
template <typename T>
geometry_msgs::Pose eigenMatrixToPoseMsg(
    const typename PointMatcher<T>::TransformationParameters &inTr) {
...
template geometry_msgs::Pose eigenMatrixToPoseMsg<float>(
    const PointMatcher<float>::TransformationParameters &inTr);
template geometry_msgs::Pose eigenMatrixToPoseMsg<double>(
    const PointMatcher<double>::TransformationParameters &inTr);

//--------------------------------
template <typename T>
tf::Transform eigenMatrixToTransform(
    const typename PointMatcher<T>::TransformationParameters &inTr) {
  tf::Transform tfTr;
  const Eigen::Affine3d eigenTr(Eigen::Matrix4d(
      eigenMatrixToDim<double>(inTr.template cast<double>(), 4)));
...
template tf::Transform eigenMatrixToTransform<float>(
    const PointMatcher<float>::TransformationParameters &inTr);
template tf::Transform eigenMatrixToTransform<double>(
    const PointMatcher<double>::TransformationParameters &inTr);

//--------------------------------
template <typename T>
tf::StampedTransform eigenMatrixToStampedTransform(
    const typename PointMatcher<T>::TransformationParameters &inTr,
    const std::string &target, const std::string &source,
    const ros::Time &stamp) {
  return tf::StampedTransform(eigenMatrixToTransform<T>(inTr), stamp, target,
                              source);
}

template tf::StampedTransform eigenMatrixToStampedTransform<float>(
    const PointMatcher<float>::TransformationParameters &inTr,
    const std::string &target, const std::string &source,
    const ros::Time &stamp);
template tf::StampedTransform eigenMatrixToStampedTransform<double>(
    const PointMatcher<double>::TransformationParameters &inTr,
    const std::string &target, const std::string &source,
    const ros::Time &stamp);
...
//--------------------------------
template <typename T>
typename PointMatcher<T>::TransformationParameters eigenMatrixToDim(
...
template PointMatcher<float>::TransformationParameters eigenMatrixToDim<float>(
